...
#Drop missing values for use in analyses
...
#Create anova dataframe of ONLY goniata and laevis:
foranov <- subset(dup_pres, sp != "pacifica")
foranov <- droplevels.data.frame(foranov, drop = TRUE)
summary(foranov)
...
ggplot(data = dup_pres, mapping = aes(x = sp, y = Depth)) +
  ...
  geom_bracket(xmin = "goniata", xmax = "laevis", label = paste0("***"), y.position = -50, size = 1, color = "black", label.size = 8, vjust = .5) +
...
#ORP differences
orp_anov <- lm(ORP_sd_100 ~ sp, data = foranov)
summary(orp_anov)
#p-value: 4.666e-10
...
ggplot(data = dup_pres, mapping = aes(x = sp, y = ORP_sd_100)) +
  ...
  geom_bracket(xmin = "goniata", xmax = "laevis", label = paste0("***"), y.position = 2.25, size = 1, color = "black", label.size = 8, vjust = .5) +
  scale_fill_grey(start = 1, end = .7) +
  theme(
    legend.title = element_blank(),
    legend.position = "none", 
...
ggplot(data = dup_pres, mapping = aes(x = sp, y = TempC)) +
  ...
  geom_bracket(xmin = "goniata", xmax = "laevis", label = paste0("***"), y.position = 6.3, size = 1, color = "black", label.size = 8, vjust = .5) +
  scale_fill_grey(start = 1, end = .7) +
  theme(
    legend.title = element_blank(),
    legend.position = "none", 
...
ggplot(data = dup_pres, mapping = aes(x = sp, y = O2)) +
  ...
  geom_bracket(xmin = "goniata", xmax = "laevis", label = paste0("***"), y.position = 122, size = 1, color = "black", label.size = 8, vjust = .5) +
  scale_fill_grey(start = 1, end = .7) +
  theme(
    legend.title = element_blank(),
    legend.position = "none", 
...
ggplot(data = dup_pres, mapping = aes(x = sp, y = PSU)) +
  ...
  geom_bracket(xmin = "goniata", xmax = "laevis", label = paste0("***"), y.position = 34.67, size = 1, color = "black", label.size = 8, vjust = .5) +
  scale_fill_grey(start = 1, end = .7) +
  theme(
    legend.title = element_blank(),
    legend.position = "none", 
...
#All species:
colnames(dup_pres)
pca.e.sub <- dup_pres[,c(1,4,5,8,9)]
pca.na <- na.omit(pca.e.sub)
colnames(pca.na)
#sp    TempC             O2      ORP_sd_100             PSU      
...
#Compile together:
pca.e <- data.frame(pcTemp.z, pcO2.z, pcORP.z, pcPSU.z)
summary(pca.e)
...
colnames(morphl)
morphl_select <- morphl[,c(4,8,9,12,13,15,16,17)]
colnames(morphl_select)
#[1] "L_Ap"        "TempC"       "O2"          "Thickness"   "ORP_sd_100" "Ap_Round"    "Shell_Round" "PSU"  
...
rdal.e <- morphl_select.na[,c(2,3,5)]
summary(rdal.e)
nrow(rdal.e)
# TempC             O2        ORP_sd_100  
#59 rows
...
#Compile together:
rdal.e.z <- data.frame(lz_TempC, lz_O2, lz_ORP)
summary(rdal.e.z)
nrow(rdal.e.z)
#59 rows
...
rdal.r <- morphl_select.na[,c(1,4,6,7)]
summary(rdal.r)
nrow(rdal.r)
...
summary(rdal)
#Variance:
#   Constrained: ~9% of the variance of Y is explained by X
#   Unconstrained: ~91% of the variance of Y is unexplained
...
ggplot()+
  ...
  geom_text(data=sc_spl,aes(x=RDA1,y=RDA2,label=rownames(sc_spl)),
            color='black', size = 5)+
  geom_segment(data = sc_bpl[c('O2'),],
               aes(x = 0, y = 0,
                   xend = RDA1,
                   yend = RDA2),
               arrow = arrow(length = unit(1/2, 'picas')),
               color = 'black', size = 1.25) +
  annotate('text',x = sc_bpl['O2','RDA1']+.4,
...
morphg_select <- morphg[,c(4,8,9,12,13,14,15,16,17)]
colnames(morphg_select)
#[1] "L_Ap"        "TempC"       "O2"          "Thickness"   "ORP_sd_100"  "rel_Texture" "Ap_Round"    "Shell_Round" "PSU"       
morphg_select.na <- na.omit(morphg_select)
summary(morphg_select.na)

#Select explanatory variables
rdag.e <- morphg_select.na[,c(2,3,5,9)]
summary(rdag.e)
nrow(rdag.e)
...
#Compile together:
rdag.e.z <- data.frame(gz_TempC, gz_O2, gz_ORP, gz_PSU)
summary(rdag.e.z)
nrow(rdag.e.z)
#55 rows
...
rdag.r <- morphg_select.na[,c(1,4,6,7,8)]
summary(rdag.r)
nrow(rdag.r)
...
summary(rdag)
#Constrained: ~14% of the variance of Y is explained by X
#Unconstrained: ~86% of the variance of Y is unexplained
...
rownames(sc_bpg) <- c('Temp.','O2','ORP','PSU')
...
ggplot()+
  ...
  geom_text(data=sc_spg,aes(x=RDA1,y=RDA2,label=rownames(sc_spg)),
            color='black', size = 5)+
  geom_segment(data = sc_bpg[c('O2','ORP'),],
               aes(x = 0, y = 0,
                   xend = RDA1,
                   yend = RDA2),
               arrow = arrow(length = unit(1/2, 'picas')),
               color = 'black', size = 1.25) +
  annotate('text',x = sc_bpg['O2','RDA1']+.3,
           y = sc_bpg['O2','RDA2'], label = 'O2',
           size = 6, color='black',fontface = 'bold')+
  annotate('text',x = sc_bpg['ORP','RDA1'],
           y = sc_bpg['ORP','RDA2']+.3, label = 'ORP',
           size = 6, color='black',fontface = 'bold')+
  labs(x='RDA1 (8.77%)*',
       y='RDA2 (2.90%)')+
  geom_hline(yintercept = 0)+
...
ap.l.o2 <- ggplot(data = morphl, mapping = aes(x = O2, y = L_Ap)) +
  geom_point(color = "red", size = 2) +
  geom_smooth(method = "lm", color = "black", se = FALSE) +
  labs(x = "O2 (uM/L)", y = "Aperture Length (mm)") +
  theme(
    legend.title = element_blank(),
    legend.text = element_text(size =12, color = "black", face = "italic"),
    legend.key = element_rect(fill = "white"),
...
grid.arrange(shr.l.o2, th.l.o2, ap.l.o2, apr.l.o2, nrow = 2, ncol = 2)
